...
const undianPromise = new Promise((resolve, reject) => {
  console.log('Sedang melakukan pengundian!');
  setTimeout(() => {
    if (Math.random() >= 0.5) {
      resolve('Kamu menang undian!!!');
    } else {
      reject(new Error('Kamu GAGAL :('));
    }
  }, 2000);
});

undianPromise.then(res => console.log(res)).catch(err => console.error(err));
